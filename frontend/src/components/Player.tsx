...
export function Player({ 
  ...
  // Strip root-motion position tracks for in-place animation
  useEffect(() => {
  ...
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      ...
      cameraAngle.current.phi = Math.max(0.15, Math.min(1.25, 
        cameraAngle.current.phi + e.movementY * 0.0015 * sensitivityRef.current
      ))
    }
  ...
  useFrame((state, delta) => {
    ...
    // Update position ref for arrow collision checks
    ...
    // Movement (disabled when frozen/answering question)
    if (!frozen) {
      ...
      } else if (animName && actions[animName]) {
    ...
    // Upward raycast for safe/danger detection
    const ray = new THREE.Raycaster(pos.clone(), new THREE.Vector3(0, 1, 0))
    const hits = ray.intersectObjects(state.scene.children, true).filter((h) =>
  ...
  return (
    <RigidBody ref={rigidBodyRef} position={position} colliders="hull" type="kinematicPosition">
